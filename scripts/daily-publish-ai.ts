...
const promptVersion = 'affiliate-editor-v4-geo-evidence'
...
function productEnrichmentData(product: ProductEnrichment) {
  ...
  const description = `${product.overview.map(clean).join(' ')} --- Key capabilities - ${capabilityText} --- Who this product is for - ${product.idealFor.map(clean).join(' - ')} --- What to verify before buying - ${product.limitations.map(clean).join(' - ')}`
  return {
...
function assertGeneratedContent(value: unknown): GeneratedContent {
  ...
  if (product.summary.length < 50 || product.summary.length > 160) throw new Error(`Product summary must be 50-160 characters (received ${product.summary.length})`)
