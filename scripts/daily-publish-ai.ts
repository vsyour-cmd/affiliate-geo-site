...
async function requestDeepSeek(product: Record<string, unknown>, recentTitles: string[], feedback: string[]) {
  ...
  const response = await fetch('https://api.deepseek.com/chat/completions', {
    ...
    body: JSON.stringify({ model, messages: [{ role: 'system', content: system }, { role: 'user', content: user }], response_format: { type: 'json_object' }, max_tokens: 8000, temperature: 0.5, stream: false }),
