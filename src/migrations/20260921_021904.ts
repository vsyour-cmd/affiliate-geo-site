...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  await db.run(sql`CREATE TABLE \`products_images_locales\` (
  	\`alt\` text,
  	\`id\` integer PRIMARY KEY NOT NULL,
  ...
  `)
  await db.run(sql`CREATE UNIQUE INDEX \`products_images_locales_locale_parent_id_unique\` ON \`products_images_locales\` (\`_locale\`,\`_parent_id\`);`)
  ...
  await db.run(sql`CREATE TABLE \`products_features_locales\` (
  	\`title\` text NOT NULL,
  	\`description\` text,
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`_locale\` text NOT NULL,
  ...
  `)
  await db.run(sql`CREATE UNIQUE INDEX \`products_features_locales_locale_parent_id_unique\` ON \`products_features_locales\` (\`_locale\`,\`_parent_id\`);`)
  ...
  await db.run(sql`CREATE TABLE \`products_geo_content\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`id\` text PRIMARY KEY NOT NULL,
  ...
  `)
  await db.run(sql`CREATE INDEX \`products_geo_content_order_idx\` ON \`products_geo_content\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`products_geo_content_parent_id_idx\` ON \`products_geo_content\` (\`_parent_id\`);`)
  await db.run(sql`CREATE INDEX \`products_geo_content_region_idx\` ON \`products_geo_content\` (\`region_id\`);`)
  await db.run(sql`CREATE TABLE \`products_geo_content_locales\` (
  	\`localized_name\` text,
  ...
  `)
  await db.run(sql`CREATE UNIQUE INDEX \`products_geo_content_locales_locale_parent_id_unique\` ON \`products_geo_content_locales\` (\`_locale\`,\`_parent_id\`);`)
  ...
  await db.run(sql`CREATE INDEX \`products_rels_geo_regions_id_idx\` ON \`products_rels\` (\`geo_regions_id\`);`)
  ...
  await db.run(sql`CREATE TABLE \`geo_regions\` (
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`code\` text NOT NULL,
  ...
  `)
  await db.run(sql`CREATE UNIQUE INDEX \`geo_regions_code_idx\` ON \`geo_regions\` (\`code\`);`)
  await db.run(sql`CREATE INDEX \`geo_regions_updated_at_idx\` ON \`geo_regions\` (\`updated_at\`);`)
  await db.run(sql`CREATE INDEX \`geo_regions_created_at_idx\` ON \`geo_regions\` (\`created_at\`);`)
  await db.run(sql`CREATE TABLE \`geo_regions_locales\` (
  	\`name\` text NOT NULL,
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`_locale\` text NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  ...
  `)
  await db.run(sql`CREATE UNIQUE INDEX \`geo_regions_locales_locale_parent_id_unique\` ON \`geo_regions_locales\` (\`_locale\`,\`_parent_id\`);`)
  await db.run(sql`CREATE TABLE \`content_updates\` (
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`type\` text NOT NULL,
  ...
  `)
  await db.run(sql`CREATE INDEX \`content_updates_updated_at_idx\` ON \`content_updates\` (\`updated_at\`);`)
  await db.run(sql`CREATE INDEX \`content_updates_created_at_idx\` ON \`content_updates\` (\`created_at\`);`)
  await db.run(sql`CREATE TABLE \`content_updates_locales\` (
  	\`title\` text NOT NULL,
  	\`content\` text,
  	\`seo_title\` text,
  	\`seo_description\` text,
  ...
  `)
  await db.run(sql`CREATE UNIQUE INDEX \`content_updates_locales_locale_parent_id_unique\` ON \`content_updates_locales\` (\`_locale\`,\`_parent_id\`);`)
  await db.run(sql`CREATE TABLE \`content_updates_rels\` (
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`order\` integer,
  	\`parent_id\` integer NOT NULL,
  	\`path\` text NOT NULL,
  ...
  `)
  await db.run(sql`CREATE INDEX \`content_updates_rels_order_idx\` ON \`content_updates_rels\` (\`order\`);`)
  await db.run(sql`CREATE INDEX \`content_updates_rels_parent_idx\` ON \`content_updates_rels\` (\`parent_id\`);`)
  await db.run(sql`CREATE INDEX \`content_updates_rels_path_idx\` ON \`content_updates_rels\` (\`path\`);`)
  await db.run(sql`CREATE INDEX \`content_updates_rels_products_id_idx\` ON \`content_updates_rels\` (\`products_id\`);`)
  await db.run(sql`CREATE INDEX \`content_updates_rels_geo_regions_id_idx\` ON \`content_updates_rels\` (\`geo_regions_id\`);`)
  ...
  await db.run(sql`CREATE TABLE \`global_supported_languages\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`id\` text PRIMARY KEY NOT NULL,
  	\`code\` text NOT NULL,
  ...
  `)
  await db.run(sql`CREATE INDEX \`global_supported_languages_order_idx\` ON \`global_supported_languages\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`global_supported_languages_parent_id_idx\` ON \`global_supported_languages\` (\`_parent_id\`);`)
  await db.run(sql`CREATE TABLE \`global_footer_links\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`id\` text PRIMARY KEY NOT NULL,
  	\`label\` text,
  ...
  `)
  await db.run(sql`CREATE INDEX \`global_footer_links_order_idx\` ON \`global_footer_links\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`global_footer_links_parent_id_idx\` ON \`global_footer_links\` (\`_parent_id\`);`)
  ...
  `)
  await db.run(sql`CREATE INDEX \`global_header_header_logo_idx\` ON \`global\` (\`header_logo_id\`);`)
  await db.run(sql`CREATE INDEX \`global_default_s_e_o_default_s_e_o_image_idx\` ON \`global\` (\`default_s_e_o_image_id\`);`)
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  ...
  await db.run(sql`DROP TABLE \`products_images_locales\`;`)
  await db.run(sql`DROP TABLE \`products_features\`;`)
  await db.run(sql`DROP TABLE \`products_features_locales\`;`)
  await db.run(sql`DROP TABLE \`products_tags\`;`)
  await db.run(sql`DROP TABLE \`products_geo_content\`;`)
  await db.run(sql`DROP TABLE \`products_geo_content_locales\`;`)
  ...
  await db.run(sql`DROP TABLE \`geo_regions\`;`)
  await db.run(sql`DROP TABLE \`geo_regions_locales\`;`)
  await db.run(sql`DROP TABLE \`content_updates\`;`)
  await db.run(sql`DROP TABLE \`content_updates_locales\`;`)
  await db.run(sql`DROP TABLE \`content_updates_rels\`;`)
  ...
  await db.run(sql`DROP TABLE \`global_supported_languages\`;`)
  await db.run(sql`DROP TABLE \`global_footer_links\`;`)
